...
def calculate_averages(nba_odds):
    games_dict = {}
...
####### FINDING VALUE BETS #######
def find_value_bets(value_bets, games_dict, threshold, home_team, away_team):
...
def main():  
    ...
    for sport in SPORTS_LIST:
        print(f'Getting {key_to_sport[sport]} odds')
        SPORTS_URL = f'https://api.the-odds-api.com/v4/sports/{sport}/odds/'
        sports_odds = get_sport_all_odds(api_key, SPORTS_URL)

        if sports_odds:
            games_dict, home_team, away_team = calculate_averages(sports_odds)
            value_bets = find_value_bets(value_bets, games_dict, threshold, home_team, away_team)
    ...
    # Sending emails if email_status is on and value_bets are found
    if email_status == "on" and value_bets:
        try:
            # Load email list
            with open('./___ValueBetEmails/email_list.json', 'r') as f:
                email_data = json.load(f)
            
            # Get Gmail service and email content
